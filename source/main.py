...
def main():
    ...
    # TODO Necessary to redefine machine_rs_data_path

    machine_rs_data_path = "../../Circuito_Almeria_171021/Iniciados/Data2"
    ...
    [y_test, [dists, y_test_pred_color]] = predict_folder(train_knn_model(df_query), df_train)

    accuracy = sum([y_test_i in np.vectorize(pos2nb_dict.get)(y_test_pred_color)[i,0] for i, y_test_i in enumerate(y_test)])/len(y_test)
    logger.info("KNN Histograms HSV Well Illuminated pixels Accuracy: " + str(round(accuracy * 100, 2)) + "%")
    accuracy = sum([y_test_i in np.vectorize(pos2nb_dict.get)(y_test_pred_color)[i,0:2] for i, y_test_i in enumerate(y_test)])/len(y_test)
    logger.info("KNN Histograms HSV Well Illuminated pixels Top2 Accuracy: " + str(round(accuracy * 100, 2)) + "%")
    accuracy = sum([y_test_i in np.vectorize(pos2nb_dict.get)(y_test_pred_color)[i,0:3] for i, y_test_i in enumerate(y_test)])/len(y_test)
